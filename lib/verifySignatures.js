...
function verifySignatures(zip) {
	return Promise.resolve(zip.entries)
		...
		.then(function (res) {
			var entries = res[0],
				signaturesXml = res[1].xml,
				signaturesRaw = res[1].raw,
				signaturesDom = res[1].dom; // @todo: can't wait for destructuring!
			...
			var isValid = sig.checkSignature(signaturesRaw.toString());
