...
const verifySignatures = function (zip) {
    ...
    return Promise.resolve(zip.entries)
        ...
        .then((res) => {
            ...
            const reference = _.indexBy(signatureElement.SignedInfo[0].Reference, '$.URI');

            const fileEntriesInReference = _.pluck(reference, '$.URI').filter(isFileEntry);
            const entriesInZip = _.pluck(signableEntries, 'fileName');
